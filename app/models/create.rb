...
class Create < ActiveRecord::Base
  ...
  def self.add_creates(manifestation_id, agent_id, type_id = [], delflg = [], newflg = false)
    if manifestation_id.blank? or agent_id.blank?
      return nil
    end
    agent_id.each_with_index do |p_id, i|
      next if delflg[i] and newflg
      sel_clm = self.find(:first, :conditions => ["work_id=? and agent_id=?", manifestation_id, p_id]) if p_id
      if sel_clm
         if delflg[i]
            sel_clm.destroy
         else
         ...
         self.create(:work_id => manifestation_id, :agent_id => p_id, :create_type_id => type_id[i])
      end
    end
  end
end
