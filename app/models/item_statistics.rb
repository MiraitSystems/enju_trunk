...
class ItemStatistics
  ...
  def make_data
    ...
    # 出力条件 ItemExinfo
    if self.output_condition.present? && Rails.application.class.parent_name == "EnjuWilmina"
...
#      second_rows = CarrierType.limit(2) # for debug
    ...
    first_rows.each do |first_row|
      if first_row == "blank"
        ...
        first_items = first_row.id ? items.where("#{first_row_column} = ?", first_row.id) : items.where("#{first_row_column} IS NULL")
      end
      second_row_details = []
