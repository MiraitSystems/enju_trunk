...
module EnjuTrunk
  module ImportBook
    SERIES_REQUIRE_COLUMNS = %w(original_title issn nacsis_identifier nbn)
    BOOK_REQUIRE_COLUMNS   = %w(original_title isbn nacsis_identifier nbn)
    BOOK_HEADER_ROW        = 1
    BOOK_DATA_ROW          = 2
    ...
    def fetch_book(field, datas, manifestation_type, item = nil, import_textresult = nil, external_resource)
      ...
      nbn = datas[field[I18n.t('resource_import_textfile.excel.book.nbn')]].to_s #ndl:JPNO,nacsis:NBN
      if external_resource == "nacsis"
        ncid = datas[field[I18n.t('resource_import_textfile.excel.book.nacsis_identifier')]].to_s
      end
